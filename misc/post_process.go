...
func processResult(net_outs []onnxruntime.TensorValue, det_scale float32) ([][]float32, [][]float32) {
	...
	for idx := range _feat_stride_fpn {
		...
		//log.Println(">det_thresh:", pos_inds)

		//log.Println("kps_preds", len(kps_preds), kps_preds[len(kps_preds)-1])

		bboxes := distance2bbox(anchor_centers, bbox_preds)
		kpss := distance2kps(anchor_centers, kps_preds)

		//log.Println("kpss", len(kpss), kpss[len(kpss)-1])
	...
	//log.Println(scores_list)
	//log.Println("kpss_list", kpss_list)
